...
PATCH = ROOT / "patches" / "dusklight-camera-fov-modifier.patch"
...
def main() -> None:
    ...
    if revision.returncode or revision.stdout.strip() != PINNED_REVISION:
        parser.error(
            f"Dusklight must be at {PINNED_REVISION}: "
            f"{revision.stderr.strip() or revision.stdout.strip()}"
        )
    ...
    if check.returncode:
        parser.error(f"camera host patch does not apply: {check.stderr.strip()}")
    if args.apply:
        applied = git(checkout, "apply", str(PATCH))
        if applied.returncode:
            parser.error(f"could not apply camera host patch: {applied.stderr.strip()}")
        reverse = git(checkout, "apply", "--reverse", "--check", str(PATCH))
        if reverse.returncode:
            parser.error(f"applied camera host patch did not verify: {reverse.stderr.strip()}")
        print(f"Applied camera host patch to {checkout}")
    else:
        print(f"Camera host patch applies to pinned Dusklight at {checkout}")
