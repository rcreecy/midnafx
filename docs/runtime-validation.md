...
23. With the CameraService 1.2 host patch applied, record camera diagnostics during outdoor and indoor traversal, targeting, combat, aiming, first person, conversation, cutscene, item-get, horseback, swimming, climbing, scene transition, detached/free camera, aspect/resolution change, disable/re-enable, and mod reload. Mode 0 exploration may report an active FOV override; every blocked context must report inactive and restore native FOV immediately. Capture matched `camera-vanilla.png` and `camera-wide.png` from the same position/orientation. Windows D3D11 mode-0 evidence exists for `F_SP103,0,27,0`; the other contexts remain required before enabling the feature by default.
