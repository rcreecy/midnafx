...
24. M7 remains at its Gate 1 research checkpoint. When a controlled geometry PoC exists, first verify Geometry disabled against the previous MidnaFX build. Identify the exact archive/BMD model used by the PoC, record its format and hook/restore logs, and capture matched original and unmistakably mutated lighting from one fixed view. Check two instances, scene unload/reload, and mod reload before treating the normal stream as proven.
