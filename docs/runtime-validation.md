...
24. M7 has a default-off, single-model Gate 1 PoC. In a source-matched Dusklight build, first verify Geometry disabled against the previous MidnaFX build. Enter a room containing the metal box (`ironbox`); `D_MN04` room 7 and `F_SP116` room 3 are candidate rooms found in the disc data, but their visible placement is unverified. Capture the original view. Enable **Mutation test: metal box only** before reloading the scene, confirm exactly one `L_mbox_00.arc/l_metabox_00.bmd` mutation log, and capture the same view with unmistakably changed lighting. Disable the toggle, reload, and capture the restored view. Verify two instances if available, archive unload/reload, and mod reload. Record restore logs and any crash or copied-instance discrepancy. The supplied June 2026 Dusklight v1.4.1 executable is incompatible with MidnaFX's pinned September 2026 SDK; do not use it for this check. **Log model catalog on resource load** remains an optional read-only aid and must be enabled before the target archive loads.
