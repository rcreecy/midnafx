...
23. With the CameraService 1.3 host patch applied, record camera diagnostics during outdoor and indoor traversal, targeting, combat, aiming, first person, conversation, cutscene, item-get, horseback, swimming, climbing, scene transition, detached/free camera, aspect/resolution change, disable/re-enable, and mod reload. Mode 0 exploration may report active FOV and chase-latitude modifiers; every blocked context must report inactive and restore native framing immediately. Capture matched `camera-vanilla.png`, `camera-wide.png`, and `camera-low.png` from the same position/orientation. Windows D3D11 mode-0 evidence exists for FOV and a 6-degree lower chase latitude at `F_SP103,0,27,0`; the other contexts and collision-heavy traversal remain required before enabling either feature by default.
