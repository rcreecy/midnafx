...
24. M7 remains at its Gate 1 research checkpoint. Enable **Log model catalog on resource load** before loading or reloading a scene; record exact archive/BMD file, node tag, normal format/stride/count, NBT and envelope status from the read-only catalog. It does not mutate normals. When a controlled geometry PoC exists, first verify Geometry disabled against the previous MidnaFX build, then record its hook/restore logs and capture matched original and unmistakably mutated lighting from one fixed view. Check two instances, scene unload/reload, and mod reload before treating the normal stream as proven.
